...
class carversavvyRosBridge(Node):
    def __init__(self):
        super().__init__('carversavvyRosBridge')

        # Node Name
        self._name = 'carversavvyRosBridge'
        ...
        with open(os.path.join(self.project_path, 'config', 'carversavvy_sensor_node_config.yaml'), 'r') as file:
            ...
            __carversavvyRosBridge = config[self._name]
            ...
            # carversavvyRosBridge Parameters
            self.imu_pub_topic = __carversavvyRosBridge['IMU_PUB_TOPIC_NAME']
            self.wheel_odom_pub_topic = __carversavvyRosBridge['WHEEL_ODOMETRY_PUB_TOPIC_NAME']
            self._WHEEL_RADIUS = __carversavvyRosBridge['WHEEL_RADIUS']
            self._BASE_WIDTH = __carversavvyRosBridge['BASE_WIDTH']
            self.odom_frame_id = __carversavvyRosBridge['ODOM_FRAME_ID']
            self.base_frame_id = __carversavvyRosBridge['BASE_FRAME_ID']
            self.imu_frame_id = __carversavvyRosBridge['IMU_FRAME_ID']
        ...
        if os.path.exists(imu_config_path):
            with open(imu_config_path, 'r') as file:
                ...
                self.get_logger().info('carversavvyRosBridge initialized with IMU calibration data.')
        else:
            self.get_logger().info('carversavvyRosBridge initialized without IMU calibration data.')
            self.gx_offset = 0.0
            self.gy_offset = 0.0
            self.gz_offset = 0.0
