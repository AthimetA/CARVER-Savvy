...
from carversavvy_interfaces.srv import carversavvyBeginOdomEKFExperiment
import yaml
import pandas as pd
import os
...
class carversavvyExperimentTestNode(Node):
    ...
    def __init__(self):
        ...
        # Wheel Odometry
        self.wheel_odom_sub = self.create_subscription(Odometry, '/carversavvy_ros/wheel_odom', self.wheel_odom_callback, 10)
        self.wheel_odom_buffer = Odometry()

        # Odometry filter
        self.odom_filter_sub = self.create_subscription(Odometry, '/carversavvy_ros/odometry', self.odom_filter_callback, 10)
        self.odom_filter_buffer = Odometry()

        # Service
        self.srv = self.create_service(carversavvyBeginOdomEKFExperiment, '/carversavvy_experiment_odom_ekf' , self.begin_calibration_callback)
        ...
        srv_text= 'ros2 service call /carversavvy_experiment_odom_ekf carversavvy_interfaces/srv/carversavvyBeginOdomEKFExperiment "{id: 0, cov: 0.025}"'
        self.get_logger().info(srv_text)

    def begin_calibration_callback(self, request: carversavvyBeginOdomEKFExperiment.Request, response: carversavvyBeginOdomEKFExperiment.Response):
    ...
    def cmd_timer_callback(self):
        if self.begin_experiment == True and self.n < self._max_n:
            ...
            folder_name =  '/home/athimet/FRA532_Exam/src/carversavvy_control/experimentresult/experiment2'
...
def main(args=None):
    ...
    node = carversavvyExperimentTestNode()
    rclpy.spin(node)
    rclpy.shutdown()
