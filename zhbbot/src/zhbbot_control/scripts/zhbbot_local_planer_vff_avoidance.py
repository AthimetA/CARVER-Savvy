...
class ZhbbotVFFNode(Node):
    ...
    def __init__(self):
        ...
        self.__LOOKAHEAD_DISTANCE = 1.5
        # Set the threshold to determine if the goal is reached
        self.__GOAL_THRESHOLD = 1.2
        # VFF controller parameters
        self.__OBSTACLE_DISTANCE = 1.1 # Threshold distance for obstacle influence
        self.__GAIN = 4.5  # Gain for the attractive vector
    ...
    def final_goal_reached(self):
        last_pose = self.path[-1]
        if np.linalg.norm([last_pose.pose.position.x - self.robot_pose.position.x, last_pose.pose.position.y - self.robot_pose.position.y]) < self.__GOAL_THRESHOLD:
            return True
        return False
    ...
    def vff_controller(self, goal_point, robot_pose):
        ...
        if self.laser_scan != None:
            ...
            max_linear_velocity = 1.0  # Maximum linear velocity
            max_angular_velocity = 0.75  # Maximum angular velocity
    ...
    def set_node_status_callback(self, request: ZhbbotSetNodeStaus.Request, response: ZhbbotSetNodeStaus.Response):
        self.get_logger().info(f'Setting node Requsted: {request.node_status}')
        ...
        self.get_logger().info(f'Node status set to: {self.node_status}')
        return response
