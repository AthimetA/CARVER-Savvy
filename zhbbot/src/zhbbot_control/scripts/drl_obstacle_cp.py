...
def transform_global_to_local(robot_pose, global_point):
    ...
    # Rotation
    local_x = delta_x * np.math.cos(-robot_theta) - delta_y * np.math.sin(-robot_theta)
    local_y = delta_x * np.math.sin(-robot_theta) + delta_y * np.math.cos(-robot_theta)

    return (local_x, local_y)

class ObstacleCP(Node):
    def __init__(self):
        super().__init__('ObstacleCP_nav2')
        ...
        self.score_pub = self.create_publisher(Float32MultiArray, '/score', 10)

        self.get_logger().info(f'Obstacle List: {self.obstacle_list}')

        # Clock subscriber
        self.control_loop_hz = 50 * SIM_SPD
        ...
        # self.CP_publisher = self.create_publisher(Obstacle, '/abwubot/obstacleCP_nav', 10)
        
        self.odom_topic_name = '/odom_groud_truth_pose'
    ...
    def publish_score(self):
        ...
        msg.data = [float(self.k_time), float(self.m_time), float(self.time_step)] 
    ...
    def clear_visual(self):
        ...
        marker = Marker()
        marker.header.frame_id = 'map'  # or the appropriate frame id for your use case
    ...
    def visualize_grouped_points(self, ob): ### point_clusters  -> self.Current_group
        ...
        for obstacle in ob: 
            CP = obstacle[0]
            obs_pose = obstacle[1]
            local_xy = transform_global_to_local(robot_pose,obs_pose)
            marker = Marker()
